...
\begin{document}
	...
	\section*{E}
	For this part, the work consists of writing the xml file specifying the cube. This can be seen in Figure \ref{fig:dimensioncube}
	\begin{figure}[h]
		\lstinputlisting[language=XML]{saikoschema.txt}
		\caption{Definition of Cube}\label{fig:dimensioncube}
	\end{figure}
	The xml file consists of the definitions of the dimensions first, whereafter the cube is defined, referencing the defined dimensions.
	For date and time, a default TimeDimension type is used, as this can specify the wanted hierachy.
	
	\section*{F}
	Do note for this section that the price is counted in danish "oere" so you have to divide by 100 to get the sale in DKK.
	In section B we wanted to see if sales increase during break times, and it can be seen that Figure \ref{fig:breaktimes} confirms this.
	
	\begin{figure}[h]
		\centering
		\includegraphics{breaktimes}
		\caption{Sales increase during break times}\label{fig:breaktimes}
	\end{figure}
	
	Next we examine the total sales per year, seen in Figure \ref{fig:salesperyear}. The sudden drop in 2008 is probably because the whole year has not been logged there yet.
	As can be seen, the sales have increased almost for each year, with a slight drop in 2003, 2005, and 2007, but this is not by a lot, as the overall tendency is an increase in sales.
	
	\begin{figure}[h]
		\centering
		\includegraphics{salesperyear}
		\caption{Sales per year}\label{fig:salesperyear}
	\end{figure}

	We also take a look at which product has been sold the most, see Figure \ref{fig:salesproduct}.
	As can be seen, soda has been sold the most, with beer on a second place.
	
	\begin{figure}[h]
		\centering
		\includegraphics{salesproduct}
		\caption{Sales per product}\label{fig:salesproduct}
	\end{figure}

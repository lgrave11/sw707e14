...
\item What should happen if a station have a failure and restarts during communication with the global system?
\end{itemize}
...
\item[Problem 1.4]
Determining what should happen when a station fails trying to communicate its current state to the global system, lies with the developer of the system.
...
But if the direction of communication is the opposite, having the global system trying to update the state of the local database of the station, the global system will have the correct state of the system.

The desired behaviour of the system is to always be in a correct state, which requires that when a communication failure happens, the part participating in the communication holding the correct state will resend its information.

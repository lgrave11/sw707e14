...
However, it solves the problem that it can become expensive for the user, as this suggestion only cost something for Aalborg Bycykel\fxwarning{Er det aalborg kommune eller?}.
...
However, it does require that the user has a mobile device that is able to scan a QR code and send the information over the internet to the global system. This could, however, be as expensive for the foreign user as the \textit{SMS} solution.
...
This would need that the user has an internet connection.
...
The \textit{SMS} solution require that the user has a phone capable of sending an SMS. 

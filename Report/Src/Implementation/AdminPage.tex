...
The \textit{traffic of bicycles during some period} can be seen using the graph in \secref{sec:}\fxwarning{Husk ref når den en gang er lavet!}
The \textit{current amount of bicycles at a given station} can be seen using the graph in \secref{sec:bicycleUsageHistory}.
How the \textit{amount of bicycles at a station change over time} can be seen in the same graph.
...
The table inserted into by the gpsregister interface described in \secref{sec:architecture}.
It is implemented by finding all the bicycles that have associated historical GPS data.
The administrator then chooses which bicycles to map, and these bicycles then have their routes loaded which are then given to Google Maps API to display as a poly line.

...
How the `amount of bicycles at a station change over time' can be seen in the line diagram in  \secref{sec:bicycleUsageHistory}.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{RouteHistory.png}
...
See \figref{fig:routehistory} for an illustration of what the page looks like.
...
The table is updated by the \texttt{GPSRegister} interface described in \secref{sec:architecture}.
...
The insertion is event-based and happens every time a bicycle is either returned to or taken from a station, and is done through the \texttt{StationToDBRegister} interface described in \secref{sec:architecture}.
...
% line diagram
To illustrate how the amount of bicycles change over time, a line diagram is generated showing time on the abscissa axis and the amount of bicycles on the ordinate axis, see \figref{fig:linediagram}.

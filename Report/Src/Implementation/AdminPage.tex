...
In this section we implement the database schema designed in \secref{sec:designAdminTools} and features using the data to visualise it in a manner making the administrator able to answer these questions.
To answer the question about \textit{which routes are used} we implemented a feature to show route history, which can be seen in \secref{sec:routeHistory}.
\textit{Hotspot detection} was not implemented, but GPS history was implemented in \secref{sec:gpsHistory}.
The \textit{traffic of bicycles during some period} can be seen using the chord diagram in \secref{sec:bicycleUsageHistory}
The \textit{current amount of bicycles at a given station} can be seen using the line diagram in \secref{sec:bicycleUsageHistory}.
...
\subsubsection{GPS Update rate}\fxwarning{Mig og Dennis synes den vakler lidt i GPS update rate. Flyt hellere fokus over på data-traffic, cache size, storage space etc.}
\begin{comment}
...
To look at time intervals the information of how often a GPS can read have to be determined.
This is determined by looking at datasheets of GPS, here a datasheet \citep{manual:gpsDataSheet} states that the GPS can read 50 times per second.

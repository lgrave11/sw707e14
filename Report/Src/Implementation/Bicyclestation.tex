...
\begin{itemize}
	\item A listener that listens for signals from the global database interface called websitetostationnotifer, see \secref{sec:webToStationI}, signalling that a booking has been created or removed.
...
The SOAP Service is used to report changes in the local data of the station to the global database, which is implemented in the stationtodbregister interface, see \secref{sec:stationToWebI}.
...
The loop starts out by finding all the expired bookings, removing the lock they had on a bicycle, telling the global database that the bookings have expired, and removing the bookings from the station database.\fxwarning{Skal referere til implementation problems?}
